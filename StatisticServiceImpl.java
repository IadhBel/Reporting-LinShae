...
public class StatisticServiceImpl extends GenericServiceImpl<Account, Statistic>implements StatisticService {
	...
	public StatisticServiceImpl(StatisticResourceAccessControl rac,
	...
	@Override
	public List<Statistic> findBetweenTwoDates(Account authUser, Account actor,
			AbstractDomain domain, String beginDate, String endDate,
			StatisticType statisticType) throws BusinessException {
		Validate.notNull(authUser, "authUser must be set.");
		checkReadPermission(authUser, actor, AccountStatistic.class,
				BusinessErrorCode.STATISTIC_FORBIDDEN, null, domain);
		Pair<Date, Date> dates = checkDatesInitialization(beginDate, endDate);
		Date bDate = dates.getLeft();
		Date eDate = dates.getRight();
		return statisticBusinessService.findBetweenTwoDates(authUser, domain,
				null, bDate, eDate, statisticType);
	}

	public Pair<Date, Date> checkDatesInitialization(String beginDate,
			String endDate) {
		...
		try {
			...
			throw new BusinessException(
					BusinessErrorCode.STATISTIC_DATE_PARSING_ERROR,
					"Can not parse the dates.");
		}
